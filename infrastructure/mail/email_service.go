...
func (s *EmailService) SendPasswordResetEmail(ctx context.Context, email, name, otpCode string) error {
	return s.sender.SendEmail(ctx, email, "Password Reset", "password_reset", map[string]any{
		"name":    name,
		"otp":     otpCode,
...
func (s *EmailService) SendWaitlistConfirmation(ctx context.Context, email, name, referralCode string, position int) error {
	return s.sender.SendEmail(ctx, email, "Welcome to the Waitlist", "waitlist_confirmation", map[string]any{
		"name":         name,
...
func (s *EmailService) SendBatchUpdate(ctx context.Context, emails []string, subject, message string) error {
	for _, email := range emails {
		if err := s.sender.SendEmail(ctx, email, subject, "batch_update", map[string]any{
			"message": message,
		}); err != nil {
			s.logger.Error("failed to send batch email", err, map[string]any{"email": email})
		}
	}
	return nil
}
