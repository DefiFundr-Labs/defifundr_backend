...
func (q *Queries) CheckWalletExists(ctx context.Context, arg CheckWalletExistsParams) (bool, error) {
	row := q.db.QueryRow(ctx, checkWalletExists, arg.WalletAddress, arg.ChainID)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
...
const getDefaultWalletByUser = `-- name: GetDefaultWalletByUser :one
SELECT id, user_id, wallet_address, wallet_type, chain_id, is_default, is_verified, verification_method, verified_at, nickname, created_at, updated_at FROM user_wallets 
WHERE user_id = $1 AND is_default = TRUE
LIMIT 1
`

func (q *Queries) GetDefaultWalletByUser(ctx context.Context, userID uuid.UUID) (UserWallets, error) {
	row := q.db.QueryRow(ctx, getDefaultWalletByUser, userID)
	var i UserWallets
	err := row.Scan(
		&i.ID,
		&i.UserID,
...
func (q *Queries) GetWalletByAddress(ctx context.Context, walletAddress string) (UserWallets, error) {
...
func (q *Queries) GetWalletByID(ctx context.Context, id uuid.UUID) (UserWallets, error) {
